...
## Çeviri sistemi (kurulu)
- **Şu an çalışan:** `haberler/index.html` özet/başlıkları tarayıcıda ücretsiz çevirir (Google gtx → MyMemory yedek), localStorage önbellekli, 🇹🇷/EN düğmesi var. Anahtar gerekmez.
- **Yüksek kalite (AI) — aktive etmek için:**
  1. Anthropic API anahtarı al (console.anthropic.com).
  2. GitHub repo → Settings → Secrets and variables → Actions → New secret: **`ANTHROPIC_API_KEY`**.
  3. Actions sekmesinden "Bilim Haberleri çeviri" workflow'unu elle çalıştır (workflow_dispatch) ya da günlük cron'u bekle.
  4. Script `data/news-tr.json` üretir (çocuk-dostu Türkçe). Site otomatik onu tercih eder; yoksa ücretsiz çeviriye düşer.
- Script: `scripts/translate-news.mjs` (model env `MODEL`, varsayılan haiku — ucuz). Workflow: `.github/workflows/news.yml` (06:00 UTC günlük).

## ⭐ Özel kayıt — gelecek ders adayı (2026-05-28)
**Barış'ın sorusu:** "Uzay neden hep yatay/kağıt gibi gösteriliyor? Dikeyde uzaya neden bakılmıyor?"
**Yanıt taslağı (gelecek dersin iskeleti):**
- Güneş Sistemi gerçekten neredeyse düzdür — protoplanetar diskin açısal momentumu nedeniyle gezegenler ekliptiğin ~7° içinde döner.
- Ama galaksi/evren 3B; bizim çizimlerimiz 2B ekrana izdüşüm.
- Uzayda "üst/alt" yoktur — sadece yerel yerçekimi tanımlar.
- Galaksimiz de bir disk (Samanyolu kenardan ince, yüzden geniş).
- Gelecek dersin içeriği: 3B Güneş Sistemi (eğimlerle), galaksi disk hem yüzden hem kenardan, gezegen yörünge eğimleri (i°) tablosu, "uzay nasıl bakılır" felsefesi.
